...
def extract_choice(text: str, think: bool) -> str:
    """Extract choice letter A-K from model output."""
    if not think:
        patterns = [
            r"\(([A-K])\)",  # (A)
            r"\[([A-K])\]",  # [A]
            r"\b([A-K])\b",  # A as a whole word
        ]
        for p in patterns:
...
def main():
    parser = argparse.ArgumentParser(description="Echo Framework Evaluation on TreeBench")
    ...
    parser.add_argument("--debug-dir", type=str, default="debug_echo_treebench")
    ...
    # Load data
    print("Loading data from TreeBench (HaochenWang/TreeBench - train split)")
    ...
    def process_item(item):
        ...
        try:
            # Add padding string to base64 if necessary
            ...
            # 由于可能包含 A到K 选项或者更少，动态拼接存在的选项
            options_str = ""
            for opt_char in "ABCDEFGHIJK":
                if opt_char in item and isinstance(item[opt_char], str) and item[opt_char].strip():
                    options_str += f"\n{opt_char}. {item[opt_char]}"
            ...
            if not args.think:
                ...
                is_correct = (choice.upper() == str(item['answer']).upper() if item['answer'] else False)
            else:
                ...
                Based ONLY on the final choice letter made by the model (ignore the difference in content of the choice), is it correct? 
    ...
    # Summary Report
    print("\n" + "=" * 50)
    print("ECHO FRAMEWORK EVALUATION REPORT (TreeBench)")
    print("=" * 50)
    total_correct = 0
    total_samples = 0
