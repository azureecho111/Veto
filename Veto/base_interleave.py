...
[Response]: """
    ...
    def generate(self, image_pil: Image.Image, question: str, custom_debug_dir: str = None) -> str:
        ...
        content.append({
            "type": "image_url",
            "image_url": {"url": f"data:image/jpeg;base64,{encode_image(state.original_image)}"}
        })
        
        # 2. 放入引导词
        content.append({
            "type": "text",
            "text": f"Here is a high-resolution image. Question: {question}. Below is the localized image for each target:\n"
        })
        ...
        for target, coords_list in state.found_targets.items():
            for box in coords_list:
                ...
                fl = max(0, gl - margin)
                ft = max(0, gt - margin)
                fr = min(orig_w, gr + margin)
                fb = min(orig_h, gb + margin)
                
                try:
                    fl, ft, fr, fb = int(fl), int(ft), int(fr), int(fb)
                    local_img = state.original_image.crop((fl, ft, fr, fb)).copy()
                except Exception:
                    local_img = state.original_image.crop((int(gl), int(gt), int(gr), int(gb))).copy()
                
                if state.debug:
                    import uuid
                    safe_target = str(target).replace(' ', '_').replace('/', '_')
                    local_img.save(os.path.join(state.output_dir, f"interleave_{safe_target}_{uuid.uuid4().hex[:4]}.jpg"))
                
                content.append({
                    "type": "image_url",
                    "image_url": {"url": f"data:image/jpeg;base64,{encode_image(local_img)}"}
                })
                
                content.append({
                    "type": "text",
                    "text": f"target:{target} location:[{int(gl)},{int(gt)},{int(gr)},{int(gb)}]\n"
                })
