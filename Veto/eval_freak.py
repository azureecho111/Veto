...
def main():
    parser = argparse.ArgumentParser(description="Enhanced Echo Evaluation ONLINE via API (FREAK with Permutation)")
    ...
    # Output config
    parser.add_argument("--output-path", type=str, default="enhanced_echo_freak_eval_results.jsonl")
    ...
    parser.add_argument("--debug-dir", type=str, default="debug_enhanced_echo_freak")
    ...
    def process_item(item):
        ...
        for p_idx, perm in enumerate(perms):
            ...
            try:
                # Issue generation request utilizing tracking
                response_text, tracker_data = model.generate_with_tracking(img_pil, question_full, custom_debug_dir=local_debug_dir)
                
                sample_tokens += tracker_data['total_tokens']
                sample_api_time += tracker_data['total_api_time']
                sample_wall_time += tracker_data['total_wall_time']
...
Respond with exactly one word: 'YES' if correct, 'NO' if incorrect."""
        ...
        print(f"########## Sample {sample_id} | Correct: {correct_count}/6 | Tokens: {sample_tokens} | Time: {sample_wall_time:.2f}s")
        ...
        res_item = {
            ...
            "metrics": {
                ...
                "total_api_time": sample_api_time,
                "total_wall_time": sample_wall_time
            }
        }
    ...
    print(f"Starting Parallel Enhanced Echo evaluation on FREAK with {args.num_workers} workers...")
    ...
    # Summary Report
    print("\n" + "="*80)
    print("ENHANCED ECHO FREAK ONLINE EVALUATION REPORT (Permutation Cycling)")
    print("="*80)
