...
def extract_targets(sentence: str):
    # 为对齐 Qwen2.5-VL-32B 版本的逻辑，从 <answer></answer> 标签中提取目标物体
    match = re.search(r"<answer>(.*?)</answer>", sentence, re.DOTALL)
    if match:
...
class EchoConfig:
    ...
    def __init__(self):
        ...
        self.ic_path = os.path.join(os.getcwd(), "ic_examples", "vstar_qwen2.5-32b.json")
...
Rules:
    ...
    3. First conduct an analysis, then summarize the answer at the end. Split single object with ',' and wrap your answer with <answer></answer> tags.
Example:
...
[Response]: """
    ...
    def ground_target_instances(self, state: RequestState, target: str):
        ...
        res = self._generate_from_vllm(p, state.current_image)
        print(f"######## {res} Let love continue")
        bbox_dict = extract_labeled_bboxes(res)
        print(f"######## {bbox_dict} LOVE RESUTL")
        ...
        if bbox_dict:
            ...
            for label, coords in bbox_dict.items():
                ...
                if self.config.verification_action_2:
                    ...
                    print(f"Original target:{target} @@@@@@@@@@@@ {v_res}")
                    ...
                    if self.config.debug:
                        with state.lock:
                            _path = os.path.join(state.output_dir, f"step_{state.step_count}_verification_focus_{label}.jpg")
                            crop_v.save(_path)
