...
INTERNVL3_PROMPTS = {
    ...
    "squeeze_efficient": """
If objects like <ref>{targets}</ref> are present near the four edges of the image, which edge area is the least likely to contain such objects? For each edge (top, bottom, left, and right), identify the largest contiguous rectangular region along that edge.
