...
from utils import encode_image, extract_labeled_bboxes_qwen3, map_coords_to_orig_qwen3, split_targets_sentence, smart_resize
import concurrent.futures

# --- Qwen3-VL Specific PROMPTS (JSON-aware) ---
# Note: Instructions explicitly ask for [x, y] or [x1, y1, x2, y2]
QWEN3_PROMPTS = {
    "extract_targets": PROMPTS["extract_targets"],
    "navigation": "For each of the target objects: {targets}, where is its most likely location in the image? Return the normalized coordinates (0-1000) for each as JSON format: {{\"bbox_2d\": [x, y], \"label\": \"target_name\"}}. Ensure you provide a separate entry for each target. If the target is not present, infer a plausible location.",
    
    "squeeze_efficient": """
If objects like {targets} are present near the four edges of the image, which edge area is the least likely to contain such objects? For each edge (top, bottom, left, and right), identify the largest contiguous rectangular region along that edge.
Output each edge's region precisely as:
top: {{\"bbox_2d\": [x1, y1, x2, y2], \"label\": \"top\"}};
bottom: {{\"bbox_2d\": [x1, y1, x2, y2], \"label\": \"bottom\"}};
left: {{\"bbox_2d\": [x1, y1, x2, y2], \"label\": \"left\"}};
right: {{\"bbox_2d\": [x1, y1, x2, y2], \"label\": \"right\"}};
""",

    "focus": "Locate ALL physical instances of '{target}' mentioned in or relevant to the question: '{question}' in this image. Return the normalized coordinates (0-1000) for each instance as JSON format: {{\"bbox_2d\": [x1, y1, x2, y2], \"label\": \"{target}\"}}. List all instances separately.",
    "verify": PROMPTS["verify"],
    "final_answer": PROMPTS["final_answer"]
}

class EchoForQwen3(EchoForQwen):
    """
    Echo Framework specifically adapted for Qwen3-VL.
    Inherits from EchoForQwen but overrides coordinate handling to use normalized (0-1000) and [x, y] order.
    """

    def __init__(self, config: EchoConfig = None):
        super().__init__(config)
        if self.config.model_name == "qwen2.5-vl-7b":
            self.config.model_name = "qwen3-vl-7b"
    ...
    def remove_unnecessary(self, state: RequestState, mode: str = "efficient"):
        ...
        if mode == "efficient":
            ...
            p_squeeze = QWEN3_PROMPTS["squeeze_efficient"].format(question=state.question, targets=all_targets)
            res_sq = self._generate_from_vllm(p_squeeze, state.current_image)
            bbox_dict = extract_labeled_bboxes_qwen3(res_sq)
    ...
    def ground_target_instances(self, state: RequestState, target: str):
        ...
        # 1. Focus Phase
        p = QWEN3_PROMPTS["focus"].format(target=target, question=state.question)
        res = self._generate_from_vllm(p, state.current_image)
        bbox_dict = extract_labeled_bboxes_qwen3(res)
        ...
        if bbox_dict:
            for label, coords in bbox_dict.items():
                ...
                if self.config.verification_action_2:
                    ...
                    v_res = self._generate_from_vllm(QWEN3_PROMPTS["verify"].format(target=target), crop_v)
        ...
        # 2. Navigation Phase
        p_nav = QWEN3_PROMPTS["navigation"].format(targets=target)
        res_nav = self._generate_from_vllm(p_nav, state.current_image)
        nav_dict = extract_labeled_bboxes_qwen3(res_nav)
        ...
        if nav_dict:
            for _, coords in nav_dict.items():
                ...
                v_res_nav = self._generate_from_vllm(QWEN3_PROMPTS["verify"].format(target=target), crop_v_nav)
    ...
    def generate(self, image_pil: Image.Image, question: str, custom_debug_dir: str = None) -> str:
        ...
        if not state.targets: return self._generate_from_vllm(question, image_pil)
        ...
        final_prompt = QWEN3_PROMPTS["final_answer"].format(question=question)
        final_content = [
