...
class EchoConfig:
    ...
    def __init__(self):
        ...
        # self.region_choose_strategy = "normal"
        self.region_choose_strategy = "normal"
        # self.region_choose_strategy = "size"
        self.max_reasoning_step = 10
...
class EchoForQwen:
    ...
    def find_targets(self, state: RequestState):
        ...
        if not os.path.exists(self.config.ic_path):
            ...
            p = PROMPTS["extract_targets"].format(question=state.question[:state.question.find("Please answer")])
...
[Response]: """
            ...
            res = self.client.chat.completions.create(model=self.config.model_name, messages=messages,
    ...
    def remove_unnecessary(self, state: RequestState, mode: str = "efficient"):
        ...
        if mode == "efficient":
            ...
            for label, coords in bbox_dict.items():
                ...
                if self.config.region_choose_strategy == 'normal':
                    print("###########I know where it is#################！")
                ...
                elif self.config.region_choose_strategy == 'random':
                    # 随机选择策略：为每个候选区域分配一个 0-1 的随机得分
                    score = random.random()
                elif self.config.region_choose_strategy == 'size':
                    # 面积优先策略：优先选择面积最大的冗余区域
                    score = (r_orig - l_orig) * (b_orig - t_orig)
            ...
            if best_bbox:
                ...
                return self._apply_pruning(state, best_bbox, target_edge=best_label,
                                           reason=f"Farthest from all predicted targets centers.")
            else:
                return False
        else:  # parallel mode
    ...
    def ground_target_instances(self, state: RequestState, target: str):
            ...
            for label, coords in bbox_dict.items():
                ...
                if self.config.verification_action_2:
                    ...
                    if self.config.debug:
                        with state.lock:
                            _path = os.path.join(state.output_dir,
                                                 f"step_{state.step_count}_verification_focus_{target}.jpg")
                            crop_v.save(_path)
        ...
        state.add_history("ground_no_result", None, f"No verified {target} detected in current view",
                          model_output=f"Focus: {res}")
        return False

    def reasoning_step(self, state: RequestState, current_reasoning_step) -> bool:
        ...
        if not self.remove_unnecessary(state, mode="efficient"):
            return True
        if current_reasoning_step >= self.config.max_reasoning_step:
            print(f"############### {current_reasoning_step} | {self.config.max_reasoning_step} ###############")
        return self.is_end_loop(state) or current_reasoning_step >= self.config.max_reasoning_step
    ...
    def generate(self, image_pil: Image.Image, question: str, custom_debug_dir: str = None) -> str:
        ...
        if not state.targets or self.config.max_reasoning_step < 0:
            return self._generate_from_vllm(question, image_pil)

        try:
            current_reasoning_step = 0
            while not self.reasoning_step(state,current_reasoning_step):
                current_reasoning_step += 1
                pass
        ...
        if self.config.visual_prompt:
            ...
            for target, box in all_boxes:
                ...
                # 转换全局到局部坐标
                rel_box = [box[0] - fl - dilate_x, box[1] - ft - dilate_y, box[2] - fl + dilate_x,
                           box[3] - ft + dilate_y]
                ...
                try:  # 增加一个白底以便于看清文字
                    ...
                    draw.rectangle((rect_bbox[0] - 2, rect_bbox[1] - 2, rect_bbox[2] + 2, rect_bbox[3] + 2),
                                   fill="white")
                except Exception:
                    pass
